...
public class ClothScrapRecipe extends SpecialCraftingRecipe {
    public ClothScrapRecipe(CraftingRecipeCategory category) {
        super(category);
    }
    ...
    public ItemStack craft(RecipeInputInventory craftingInventory, DynamicRegistryManager registryManager) {
        ...
        if(list[0] != -1 && list[1] != -1) {
            return switch(((ClothItem)craftingInventory.getStack(list[0]).getItem()).getClothEntry().getClothRarity()) {
    ...
    private static int[] processInventory(RecipeInputInventory craftingInventory) {
        ...
        return new int[]{clothIndex, shearsIndex};
    }
    ...
    public RecipeSerializer<?> getSerializer() {
        return ModRegistry.CLOTHSCRAP_SERIALIZER;
    }
}
