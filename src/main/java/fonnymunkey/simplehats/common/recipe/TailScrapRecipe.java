...
public class TailScrapRecipe extends SpecialCraftingRecipe {
    public TailScrapRecipe(CraftingRecipeCategory category) {
        super(category);
    }
    ...
    public ItemStack craft(RecipeInputInventory craftingInventory, DynamicRegistryManager registryManager) {
        ...
        if(list[0] != -1 && list[1] != -1) {
            return switch(((TailItem)craftingInventory.getStack(list[0]).getItem()).getTailEntry().getTailRarity()) {
    ...
    private static int[] processInventory(RecipeInputInventory craftingInventory) {
        ...
        return new int[]{tailIndex, shearsIndex};
    }
    ...
    public RecipeSerializer<?> getSerializer() {
        return ModRegistry.TAILSCRAP_SERIALIZER;
    }
}